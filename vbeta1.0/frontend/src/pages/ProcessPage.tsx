...
import { formatAnalysisJobLabel, formatJobDate } from "../jobPresentation";
...
function stepIndex(status: string): number {
  ...
  if (status === "FALHA_VALIDACAO") return 1;
  if (status === "FALHA_SOLVER") return 2;
  if (status === "FALHA_AUDITORIA") return 3;
  if (status.startsWith("FALHA")) return 0;
...
export default function ProcessPage() {
  ...
  const jobFailed = Boolean(job?.terminal && job.status !== "CONCLUIDA");
  ...
  return (
    <div className="page-container">
              ...
              <select value={job?.id ?? ""} onChange={(event) => void openExistingJob(event.target.value)}>
                ...
                {jobs.map((item) => (
                  <option value={item.id} key={item.id}>
                    {formatAnalysisJobLabel(item)} · {humanizeStatus(item.status)}
                  </option>
                ))}
              </select>
            </label>
      ...
      {job && (
        <section className="panel" aria-busy={!job.terminal}>
          <div className="section-title">
          ...
          <div className="job-meta">
            ...
            <span>Início {formatJobDate(job.created_at)}</span>
          </div>
          <ol className="stepper">
            {steps.map((step, index) => (
              <li className={jobFailed && index === currentStep ? "failed" : index < currentStep ? "done" : index === currentStep ? "current" : ""} key={step}>
                <span>{jobFailed && index === currentStep ? "!" : index < currentStep ? "✓" : index + 1}</span>
                <strong>{step}</strong>
              </li>
            ))}
          </ol>
          <div className={`process-message stage-transition ${jobFailed ? "is-failure" : ""}`} key={job.status} role="status" aria-live="polite">
      ...
      {summary && job && (
        <section className="panel result-section">
          <div className="section-title">
            ...
            <StatusBadge value={summary.solver_status} label={`CP-SAT ${humanizeStatus(summary.solver_status)}`} />
          </div>
          ...
          <div className="table-toolbar">
            ...
            <form onSubmit={submitSearch} aria-label="Filtrar decisões de alocação">
              <label className="sr-only" htmlFor="allocation-status-filter">Status da alocação</label>
              <select id="allocation-status-filter" value={allocationStatus} onChange={(event) => { setPage(1); setAllocationStatus(event.target.value); }}>
                <option value="">Todos os status</option>
                <option value="ALOCADA">Alocadas</option>
                <option value="NAO_ALOCADA">Não alocadas</option>
              </select>
              <label className="sr-only" htmlFor="allocation-search">Buscar disciplina, docente ou chapa</label>
              <input id="allocation-search" type="search" placeholder="Disciplina, docente ou chapa" value={searchDraft} onChange={(event) => setSearchDraft(event.target.value)} />
                  ...
                  <tbody>
                    {allocations.items.map((item) => (
                      <tr key={item.transmission_id}>
                        <td><span className={`allocation-status ${item.status.toLowerCase()}`}>{humanizeStatus(item.status)}</span></td>
