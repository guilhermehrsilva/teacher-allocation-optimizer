...
from .services.scenarios import (
    ...
    add_scenario_policy,
    compare_scenario,
    delete_scenario_change as remove_scenario_change,
    delete_scenario_policy as remove_scenario_policy,
    public_scenario,
    source_catalog,
)
...
from .security import LocalRequestSecurityMiddleware
from .services.coordination import LifecycleCoordinator
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    with coordinator.exclusive():
        cleanup_upload_storage(settings, database)
    manager = JobManager(settings, database, coordinator)
    scenario_manager = ScenarioManager(settings, database, coordinator)
    ...
    app.state.lifecycle_coordinator = coordinator
    ...
    async def frontend_cache_policy(request: Request, call_next):
        ...
        if not request.url.path.startswith("/api/"):
            if request.url.path.startswith("/assets/"):
                # Vite assets carry a content-hash in their filename and are
                # safe to cache indefinitely; the HTML shell references them
                # by their hashed name, so a new build automatically
                # invalidates the browser cache.
                response.headers["Cache-Control"] = "public, max-age=31536000, immutable"
            else:
                # The HTML shell and any other non-API routes must always be
                # revalidated so the browser fetches updated asset references.
    ...
    def upload_and_validate(
        ...
        module: Annotated[int, Form(ge=51, le=54)],
    ) -> dict:
        try:
            with coordinator.exclusive():
    ...
    def create_job(payload: JobCreate) -> dict:
        ...
        except JobManagerUnavailable as exc:
    ...
    def reset_primary_jobs(
        ...
        try:
            with scenario_manager.exclusive():
    ...
    def create_scenario(payload: ScenarioCreate) -> dict:
        ...
        if not name:
            raise HTTPException(status_code=422, detail="Informe um nome para o cenário.")
        with scenario_manager.exclusive():
    ...
    def delete_scenario_change(scenario_id: str, change_id: str) -> None:
        try:
            with scenario_manager.exclusive():
                if not remove_scenario_change(database, scenario_id, change_id):
    ...
    def delete_scenario_policy(scenario_id: str, policy_id: str) -> None:
        try:
            with scenario_manager.exclusive():
                if not remove_scenario_policy(database, scenario_id, policy_id):
    ...
    def run_scenario(scenario_id: str) -> dict:
        ...
        except ScenarioQueueFull as exc:
            raise HTTPException(
                status_code=429,
                detail=str(exc),
                headers={"Retry-After": "5"},
