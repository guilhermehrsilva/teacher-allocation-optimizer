...
from .jobs import public_job, read_json, verified_manifest_artifact
...
def _allocation_workbook(job: dict[str, Any]) -> Path:
    ...
    return verified_manifest_artifact(round_dir, manifest, "allocation_workbook")


def _teacher_identity(row: dict[str, Any], name_field: str) -> str:
    """Use the stable badge, falling back to a normalized name for old output."""
    badge = _text(row.get("CHAPA"), "")
    if badge:
        return f"CHAPA:{badge}"
    return f"NOME:{_text(row.get(name_field), '').casefold()}"
...
def build_insights(job: dict[str, Any]) -> dict[str, Any]:
    ...
    allocation_names: dict[str, str] = {}
    for row in allocated:
        identity = _teacher_identity(row, "DOCENTE")
        allocation_counts[identity] += 1
        allocation_names.setdefault(identity, _text(row.get("DOCENTE")))
    teacher_lookup = {_teacher_identity(row, "NOME"): row for row in teachers}
    teacher_days: dict[str, Counter[str]] = defaultdict(Counter)
    for row in allocated:
        teacher_days[_teacher_identity(row, "DOCENTE")][_text(row.get("DIA_AULA"))] += 1
    ...
    for identity, count in ranked_counts:
        previous_cumulative = cumulative
        cumulative += count
        cumulative_pct = 100 * cumulative / total_allocated if total_allocated else 0
        if total_allocated and previous_cumulative < 0.8 * total_allocated:
            pareto_count += 1
        row = teacher_lookup.get(identity, {})
        name = _text(row.get("NOME"), allocation_names.get(identity, "Não informado"))
    ...
    for identity, count in ranked_counts:
        ...
        day_peak = max(teacher_days[identity].values(), default=0)
        ...
        risk_teachers.append(
            {
                ...
                "load_outlier": identity in outliers,
            }
        )
    ...
    cluster_coverage = _coverage_rows(allocations, ("CLUSTER",))
    for item in cluster_coverage:
        if not item.get("cluster"):
            item["cluster"] = "Não informado"
