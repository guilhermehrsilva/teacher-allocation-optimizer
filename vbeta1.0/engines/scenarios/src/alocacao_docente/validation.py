...
VALIDATION_CONTRACT_VERSION = "2.2"
...
def _validate_mapa(
    ...
    # ORDEM define as janelas de capacidade consumidas pelo solver. Portanto,
    # seu domínio é bloqueante apenas nas ofertas que chegam ao motor; em
    # linhas informativas, eventual inconsistência continua sendo ressalva.
    _check_enums(
        report,
        MAPA_SHEET,
        records,
        {column: allowed for column, allowed in MAPA_ENUMS.items() if column != "ORDEM"},
    )
    order_enum = {"ORDEM": MAPA_ENUMS["ORDEM"]}
    _check_enums(report, MAPA_SHEET, allocating, order_enum)
    _check_enums(report, MAPA_SHEET, supporting, order_enum, blocking=False)
