...
PIPELINE_VERSION = "0.1.0-scenario"
OUTPUT_SCHEMA_VERSION = 2
DEFAULT_EXPECTED_MODULE = 52
RANDOM_SEED = 42
GRASP_ITERATIONS = 0
...
DEFAULT_OUTPUT_DIR = ROOT / "resultados"
...
class PipelineConfig:
    ...
    expected_module: int = DEFAULT_EXPECTED_MODULE
...
def _artifact_payload(path: Path, round_dir: Path) -> dict[str, Any]:
    return {
...
def _code_fingerprints() -> dict[str, dict[str, str]]:
    ...
    component_dir = SOURCE_DIR / "motor_alocacao"
    files = {
        "pipeline": Path(__file__).resolve(),
        "validation": SOURCE_DIR / "alocacao_docente" / "validation.py",
        ...
        "policies": component_dir / "policies.py",
...
def _copy_policy_snapshot(
    ...
    """Freeze and validate the exact scenario premises consumed by the solver."""
    source = source.resolve()
    hash_before = _sha256(source)
    destination_dir = round_dir / "premissas"
    destination_dir.mkdir(parents=True, exist_ok=True)
    destination = destination_dir / "cenario.json"
    temporary = destination.with_name(f".{destination.name}.{uuid.uuid4().hex}.tmp")
    try:
        shutil.copy2(source, temporary)
        temporary.replace(destination)
        ...
        temporary.unlink(missing_ok=True)
    copied_hash = _sha256(destination)
    if hash_before != copied_hash or hash_before != _sha256(source):
        raise InputConfigurationError(
            "As premissas do cenário foram alteradas durante a cópia; execute novamente."
        )
    try:
        payload = json.loads(destination.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise InputConfigurationError(
            "O snapshot de premissas do cenário não é um JSON válido."
        ) from exc
    if not isinstance(payload, dict):
        raise InputConfigurationError("O snapshot de premissas deve ser um objeto JSON.")
    return destination, copied_hash, payload
...
def _config_payload(config: PipelineConfig) -> dict[str, Any]:
    return {
        ...
        "policy_path": str(Path(config.policy_path).resolve()) if config.policy_path else None,
    }
...
def _validate_config(config: PipelineConfig) -> None:
    if (
        not isinstance(config.expected_module, int)
        or isinstance(config.expected_module, bool)
        or config.expected_module < 1
    ):
        raise InputConfigurationError("expected_module deve ser um inteiro positivo.")
    if (
    ...
    if config.policy_path is not None and not Path(config.policy_path).is_file():
        raise InputConfigurationError("O arquivo de políticas do cenário não existe.")
    ...
    try:
        ...
        try:
            ...
            if config.policy_path:
                policy_snapshot, policy_hash, scenario_policy = _copy_policy_snapshot(
        ...
        manifest["artifacts"]["source_copy"] = _artifact_payload(
            copied_source,
            round_dir,
        )
        if policy_snapshot is not None:
            manifest["artifacts"]["scenario_policy_snapshot"] = _artifact_payload(
                policy_snapshot,
                round_dir,
            )
            manifest["config"]["policy_sha256"] = policy_hash
        _atomic_write_json(manifest_path, manifest)

        status.update(
            "VALIDANDO",
            f"Validando a cópia da base para o módulo {config.expected_module}.",
        )
        try:
        ...
        manifest["artifacts"].update(
            {
                "validation_report": _artifact_payload(validation_json, round_dir),
                "validation_issues": _artifact_payload(validation_csv, round_dir),
            }
        )
        _atomic_write_json(manifest_path, manifest)
        if validation.status == "REPROVADO":
            raise _StageFailure(
                EXIT_VALIDATION,
                "VALIDACAO_REPROVADA",
                f"A base foi reprovada pela validação do módulo {config.expected_module}.",
            )
        ...
        manifest["phases"]["solver"] = {
            ...
            "scenario_policies": len((scenario_policy or {}).get("policies", [])),
        ...
        try:
            audit = audit_allocation(problem, result, scenario_policy)
        ...
        manifest["artifacts"]["audit_report"] = _artifact_payload(
            audit_path,
            round_dir,
        )
        _atomic_write_json(manifest_path, manifest)
        ...
        manifest["artifacts"].update(
            {
                "allocation_workbook": _artifact_payload(allocation_xlsx, round_dir),
                "allocation_summary": _artifact_payload(allocation_json, round_dir),
            }
        )
        _atomic_write_json(manifest_path, manifest)
