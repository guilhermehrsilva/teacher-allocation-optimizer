...
class PipelineTests(unittest.TestCase):
    ...
    def test_cli_can_run_by_absolute_path_from_another_directory(self):
        workspace = self.make_workspace()
        completed = subprocess.run(
            [sys.executable, str((ROOT / "executar_cenario.py").resolve()), "--help"],
            cwd=workspace,
            capture_output=True,
            text=True,
            check=False,
        ...
        self.assertIn("--modulo", completed.stdout)

    def test_blank_order_is_rejected_before_loader_and_solver(self):
        workspace = self.make_workspace()
        input_dir = workspace / "entrada"
        output_dir = workspace / "resultado"
        input_dir.mkdir()
        source = input_dir / "base_sem_ordem.xlsx"
        write_valid_m52_workbook(source)
        workbook = load_workbook(source)
        try:
            workbook[MAPA_SHEET].cell(
                row=2,
                column=MAPA_HEADERS.index("ORDEM") + 1,
            ).value = None
        ...
        with (
            patch("pipeline.load_problem") as load_problem_mock,
            patch("pipeline.solve_allocation") as solve_allocation_mock,
        ):
            run = run_pipeline(PipelineConfig(input_dir=input_dir, output_dir=output_dir))

        self.assertEqual(EXIT_VALIDATION, run.exit_code, run.message)
        self.assertEqual("VALIDACAO_REPROVADA", run.state)
        load_problem_mock.assert_not_called()
        solve_allocation_mock.assert_not_called()
        assert run.round_dir is not None
        report = json.loads(
            (run.round_dir / "validacao" / "relatorio_validacao.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertTrue(
            any(
                issue["code"] == "CAMPO_OBRIGATORIO_VAZIO"
                and issue["column"] == "ORDEM"
                and issue["blocking"]
    ...
    def test_valid_m52_pipeline_is_optimal_and_audited(self):
        ...
        self.assertEqual(2, manifest["versions"]["output_schema"])
        ...
        for fingerprint in manifest["code_fingerprints"].values():
            code_path = ROOT / fingerprint["path"]
        ...
        self.assertEqual("CLT EAD", summary["decisions"][0]["contract_model"])
        self.assertNotIn("original_contract_model", summary["decisions"][0])
        self.assertNotIn("suggested_contract_model", summary["decisions"][0])
        self.assertNotIn("contract_model_comparison", summary)
        ...
        for artifact in manifest["artifacts"].values():
            artifact_path = round_dir / artifact["path"]
        ...
        try:
            ...
            self.assertNotIn("MODELO_CONTRATO_ORIGEM", headers)
    ...
    def test_policy_snapshot_is_copied_hashed_and_fingerprinted(self):
        workspace = self.make_workspace()
        source = workspace / "base_m52.xlsx"
        output_dir = workspace / "resultado"
        policy_path = workspace / "alteracoes.json"
        write_valid_m52_workbook(source)
        policy_payload = {
            "schema_version": 1,
            "scenario_id": "cenario-teste",
            "changes": [],
            "policies": [],
        }
        ...
        assert run.round_dir is not None
        manifest = json.loads(
            (run.round_dir / "manifesto.json").read_text(encoding="utf-8")
        )
        artifact = manifest["artifacts"]["scenario_policy_snapshot"]
        snapshot = run.round_dir / artifact["path"]
        self.assertEqual(policy_payload, json.loads(snapshot.read_text(encoding="utf-8")))
        self.assertEqual(hashlib.sha256(snapshot.read_bytes()).hexdigest(), artifact["sha256"])
        self.assertEqual(artifact["sha256"], manifest["config"]["policy_sha256"])
        self.assertIn("policies", manifest["code_fingerprints"])
