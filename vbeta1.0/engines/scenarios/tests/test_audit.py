        ...
        self.assertEqual(
            {
                ...
                "scenario_policies_checked": 0,
            },
            audit.checks,
        )

    def test_rejects_unmet_priority_policy(self):
        problem = Problem(Path("test.xlsx"), (teacher(),), (transmission(0),))
        assignment = Assignment(
            transmission_id=0,
            teacher_id=None,
            status="NAO_ALOCADA",
            reason="Sem docente disponível",
        ...
        audit = audit_allocation(problem, result([assignment], {0: 0}), policy)

        self.assert_rejected_with(audit, "PRIORIDADE_NAO_ATENDIDA")

    def test_rejects_unmet_fixed_assignment_policy(self):
        problem = Problem(
            Path("test.xlsx"),
            (teacher(0), teacher(1)),
            (transmission(0),),
            ...
            "policies": [
                {
                    ...
                    "target_type": "OFFER",
                    "target_value": "2",
                    "configuration": {"teacher_badge": teacher(1).badge},
                }
            ]
        }
        ...
        audit = audit_allocation(
            problem,
            result(
                [assigned(0, eligible_teacher_count=2)],
                {0: 2, 1: 0},
                {
                    0: {MODULE_STAGE_1: 2, MODULE_STAGE_2: 0},
                    1: {MODULE_STAGE_1: 0, MODULE_STAGE_2: 0},
                },
            ),
            policy,
        )

        self.assert_rejected_with(audit, "ALOCACAO_FIXADA_NAO_ATENDIDA")
