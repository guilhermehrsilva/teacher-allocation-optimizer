...
export default function InsightsPage() {
  ...
  useEffect(() => {
    ...
    setInsights(null);
    setError("");
    void getInsights(selected).then(setInsights).catch((requestError: Error) => setError(requestError.message));
  }, [selected]);
  ...
  return (
    <div className="page-container">
      <div className="page-heading">
        ...
        <div className="page-heading-actions"><label className="compact-field"><span>Rodada analisada</span><select value={selected} onChange={(event) => setSelected(event.target.value)}>{jobs.length === 0 && <option value="">Nenhuma rodada concluída</option>}{jobs.map((job) => <option key={job.id} value={job.id}>{job.round} · {job.filename}</option>)}</select></label><ResetPrimaryRounds disabled={!jobs.some((job) => job.kind === "PRIMARY")} /></div>
      </div>
      {error && <div className="alert error" role="alert">{error}</div>}
      {!insights && !error && <section className="empty-state panel"><span>✦</span><h2>Execute uma rodada para gerar os insights</h2><p>A análise usa somente o resultado auditado.</p><a className="button primary" href="#/processamento">Ir para processamento</a></section>}
      ...
      {insights && <>
        ...
        <section className="panel">
          <div className="section-title"><div><span className="step-kicker">DISCIPLINAS</span><h2>Fila de recuperação de cobertura</h2><p>Disciplinas com lacunas, ordenadas pelo maior impacto.</p></div></div>
          <div className="table-wrap"><table><thead><tr><th>Curso</th><th>Disciplina</th><th>Cobertura</th><th>Não alocadas</th><th>Demanda</th><th>Candidato único</th></tr></thead><tbody>{insights.coverage.discipline_gaps.filter((item) => item.unassigned > 0).slice(0, 30).map((item) => <tr key={`${item.curso}-${item.cod_disciplina}`}><td><strong>{item.nome_curso}</strong><small>{item.curso}</small></td><td><strong>{item.nome_disciplina}</strong><small>{item.cod_disciplina}</small></td><td><span className={`coverage-pill ${item.coverage_pct === 0 ? "critical" : "attention"}`}>{item.coverage_pct.toFixed(1)}%</span></td><td>{item.unassigned}</td><td>{item.demand_hours}h</td><td>{item.single_candidate}</td></tr>)}</tbody></table></div>
        </section>
        ...
        <footer className="source-note"><span>Fonte</span>{insights.source_note}</footer>
      </>}
    </div>
  );
}
