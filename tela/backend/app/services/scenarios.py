...
from .jobs import load_summary, public_job, read_json
...
SCENARIO_POLICY_TYPES = {"ALOCAR_CLUSTER", "INTERNALIZACAO", "PRIORIDADE", "FIXAR"}
...
def _normalize_change_value(field: str, value: Any) -> Any:
    ...
    if field == "DIA_AULA":
        ...
        if normalized not in {
            "SEGUNDA", "TERÇA", "QUARTA", "QUINTA", "SEXTA", "SÁBADO", "DOMINGO", "NSA",
        }:
...
def _baseline_source(database: Database, scenario: dict[str, Any]) -> Path:
    ...
    path = Path(baseline["stored_path"]).resolve()
    if not path.is_file():
        raise ScenarioError("A planilha original da rodada-base não está disponível.")
    return path
...
def add_scenario_change(
    ...
    *,
    ...
    database.upsert_scenario_change(payload)
    database.update_scenario(scenario_id, status="RASCUNHO")
    return public_change(payload)
...
def add_scenario_policy(
    ...
    *,
    ...
    database.upsert_scenario_policy(payload)
    database.update_scenario(scenario_id, status="RASCUNHO")
...
class ScenarioManager:
    ...
    def create_run(self, scenario_id: str) -> dict[str, Any]:
        ...
        self.executor.submit(self._execute, scenario_id, job_id, input_path)
    ...
    def _execute(self, scenario_id: str, job_id: str, input_path: Path) -> None:
        ...
        try:
            ...
            self.database.update_job(
                job_id,
                status="ERRO_INTERNO",
                message=f"Falha no motor secundário: {exc}",
                exit_code=50,
            )
