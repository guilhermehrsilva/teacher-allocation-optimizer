...
def build_parser() -> argparse.ArgumentParser:
    parser = _ArgumentParser(
        description=(
            "Valida, resolve e audita uma simulação isolada de alocação docente. "
            "Este executável não é utilizado pelo processamento principal."
        ),
    )
    parser.add_argument(
        "--base",
    ...
    parser.add_argument(
        "--modulo",
        type=int,
        default=DEFAULT_EXPECTED_MODULE,
        help=f"Módulo esperado na base (padrão: {DEFAULT_EXPECTED_MODULE}).",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="JSON auditável com as políticas exclusivas desta simulação.",
    )
    parser.add_argument(
        "--permitir-nao-otimo",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    run = run_pipeline(
        PipelineConfig(
            ...
            expected_module=args.modulo,
            policy_path=args.politicas,
        )
    )
