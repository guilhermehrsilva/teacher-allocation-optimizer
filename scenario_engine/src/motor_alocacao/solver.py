...
def solve_allocation(
    ...
    model = cp_model.CpModel()
    eligible = build_eligibility(problem)
    assignment_vars: dict[tuple[int, int], cp_model.IntVar] = {}
    ...
    policies = list((scenario_policy or {}).get("policies", []))
    cluster_targets = {
        str(policy.get("target_value") or "")
    ...
    cluster_override_pairs: set[tuple[int, int]] = set()
    for transmission in problem.transmissions:
        if transmission.cluster not in cluster_targets or transmission.slot is None:
            continue
        profiles_in_cluster = cluster_profiles[transmission.cluster]
        for teacher in problem.teachers:
            if (
                teacher.id not in eligible[transmission.id]
                ...
                eligible[transmission.id].append(teacher.id)
                cluster_override_pairs.add((transmission.id, teacher.id))
    ...
    internalization_enabled = any(
        str(policy.get("policy_type") or "").upper() == "INTERNALIZACAO"
        for policy in policies
    )
    internal_score = sum(
        variable
        for (_, teacher_id), variable in assignment_vars.items()
        if problem.teachers[teacher_id].contract_family != CONTRACT_UNKNOWN
    )
    internal_solver = None
    internal_status = None
    internal_incumbent = second_incumbent
    if internalization_enabled:
        model.maximize(internal_score)
        internal_solver = _new_solver(max_time_seconds, 0.15, workers, random_seed + 1)
        internal_status = internal_solver.solve(model)
        if internal_status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            best_internal_score = int(round(internal_solver.objective_value))
            internal_incumbent = internal_solver
        else:
            best_internal_score = int(second_incumbent.value(internal_score))
        model.add(internal_score == best_internal_score)
        _replace_hints(model, internal_incumbent, all_variables)
    else:
        best_internal_score = int(second_incumbent.value(internal_score))
    ...
    third_solver = _new_solver(max_time_seconds, 0.20, workers, random_seed + 2)
    ...
    elif internal_incumbent:
        best_capacity_score = int(internal_incumbent.value(high_capacity_score))
    else:
        best_capacity_score = int(first_solver.value(high_capacity_score))
    third_incumbent = (
        third_solver
        if third_status in (cp_model.OPTIMAL, cp_model.FEASIBLE)
        else internal_incumbent
    )
    ...
    fourth_solver = _new_solver(max_time_seconds, 0.15, workers, random_seed + 3)
    ...
    if internal_status is not None:
        phase_statuses.append(internal_status)
    ...
    return AllocationResult(
        ...
        diagnostics={
            ...
            "scenario_internalization_status": (
                _status_name(internal_status) if internal_status is not None else "NAO_SOLICITADA"
            ),
            "scenario_internal_assignments": best_internal_score,
