...
def audit_allocation(problem: Problem, result: AllocationResult) -> AllocationAudit:
    ...
    eligibility = build_eligibility(problem)
    ...
    scenario_eligibility = result.diagnostics.get("scenario_cluster_eligibility", {})
    scenario_override_pairs = {
        (int(pair[0]), int(pair[1]))
        for pair in result.diagnostics.get("scenario_cluster_override_pairs", [])
    ...
    for transmission_id in sorted(expected_ids & set(assignments_by_transmission)):
        ...
        eligible.update(
            int(teacher_id)
            for teacher_id in scenario_eligibility.get(str(transmission_id), [])
        )
