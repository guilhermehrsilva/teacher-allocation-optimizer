...
MAPA_ENUMS = {
    ...
    "MODELO_CONTRATO": {
        "CLT EAD", "CLT STRICTO", "STRICTO SENSU", "CLT STRICTO SENSU",
        "RPA/NF", "SOB DEMANDA",
    },
...
def _validate_mapa(
    ...
    remaining_required = tuple(
        ...
        if header not in routing_columns + allocation_columns and header != "HORÁRIO"
    )
    ...
    operational_enums = {
        column: allowed
        for column, allowed in MAPA_ENUMS.items()
        if column != "MODELO_CONTRATO"
    }
    _check_enums(report, MAPA_SHEET, records, operational_enums)
    _check_enums(
        report,
        MAPA_SHEET,
        records,
        {"MODELO_CONTRATO": MAPA_ENUMS["MODELO_CONTRATO"]},
        blocking=False,
    )
...
def validate_workbook(
    ...
    report.assumptions = [
        ...
        "MODELO_CONTRATO da disciplina é histórico e não restringe a elegibilidade; o modelo sugerido deriva do docente selecionado.",
