...
class ValidationTests(unittest.TestCase):
    ...
    def test_historical_contract_does_not_restrict_profile_coverage(self):
        mapa = valid_mapa_row()
        mapa[18] = "CLT STRICTO"
    ...
    def test_unknown_historical_contract_is_non_blocking(self):
        mapa = valid_mapa_row()
        mapa[18] = "MODELO LEGADO"
        ...
        issues = [
            issue
            for issue in report.issues
            if issue.code == "VALOR_FORA_DOMINIO"
            and issue.column == "MODELO_CONTRATO"
        ]
        self.assertEqual("APROVADO_COM_RESSALVAS", report.status)
        self.assertEqual(1, len(issues))
        self.assertFalse(issues[0].blocking)

    def test_missing_historical_contract_is_non_blocking(self):
        mapa = valid_mapa_row()
        mapa[18] = None
