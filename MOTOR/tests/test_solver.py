...
from motor_alocacao import (  # noqa: E402
    Problem, Teacher, Transmission, build_grasp_hint, contract_model_for,
    solve_allocation,
)
...
def transmission(
    ...
    contract_model="CLT EAD", order="1ª",
) -> Transmission:
    return Transmission(
        ...
        day, start, order, "GESTÃO", "Coordenador", contract_model,
    )
...
class SolverTests(unittest.TestCase):
    ...
    def test_stricto_teacher_with_zero_base_capacity_gets_up_to_two_disciplines(self):
        ...
        result = self.solve(
            [stricto],
            [
                transmission(0, contract_model="CLT STRICTO"),
                transmission(
                    1, start=time(20, 40), contract_model="CLT STRICTO", order="2ª",
                ),
                transmission(
                    2, day="TERÇA", contract_model="CLT STRICTO", order="2ª",
                ),
            ],
        )
        self.assertEqual(2, result.allocated_count)
    ...
    def test_historical_contract_does_not_restrict_teacher_selection(self):
        ...
        result = self.solve(
            ...
            [
                transmission(
                    ...
                    contract_model="CLT STRICTO",
                ),
                transmission(
                    1,
                    start=time(20, 40),
                    profiles=presencial_profile,
                    contract_model="CLT EAD",
                ),
            ],
        )
    ...
    def test_unknown_historical_contract_does_not_block_allocation(self):
        result = self.solve(
            [teacher(0)],
            [transmission(0, contract_model="MODELO LEGADO")],
        )

        self.assertEqual(0, result.assignments[0].teacher_id)
        self.assertFalse(
            result.diagnostics["historical_contract_restricts_eligibility"]
        )
    ...
    def test_grasp_allows_two_stricto_disciplines(self):
        ...
        transmissions = (
            transmission(0, contract_model="CLT STRICTO"),
            transmission(1, start=time(20, 40), contract_model="CLT STRICTO"),
        )
    ...
    def test_grasp_keeps_stricto_limit_across_module_stages(self):
        ...
        transmissions = (
            transmission(0, contract_model="CLT STRICTO", order="1ª"),
            transmission(
                1,
                start=time(20, 40),
                contract_model="CLT STRICTO",
